...
export async function POST(req:Request) {
     try {
        ...
        const response: Partial<makepayresponse> = await axios.post('https://new.techpay.co.zm/api/v1/ic/pay/mobilemoney', {
        ...
        if (response.responsecode == 100) {
            ...
            const paymentStatus: any = await pollPaymentStatus(orderToken);

            return new Response(JSON.stringify({ success: true, paymentStatus, data: order_id }), {
                status:200
            });

        } else {
            return new Response(JSON.stringify({ success: false, message: "Payment initiation failed" }), {
                status: 422
            });
        }
